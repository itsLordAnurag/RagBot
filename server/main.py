...
def upload_pdfs(session_id: str, files:List[UploadFile]=File(...)):
    try:
        logger.info(f"recieved {len(files)} files for session {session_id}")
        load_vectorstore(files, session_id)
...
def clear_db(session_id: str):
    try:
        ...
        logger.info(f"vectorstore cleared successfully for session {session_id}")
...
@app.post("/ask/")
def ask_question(session_id: str, question: str = Form(...), groq_api_key: str = Form(None)):
    try:
        logger.info(f"user query: {question} for session {session_id}")
